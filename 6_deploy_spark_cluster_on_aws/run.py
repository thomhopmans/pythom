...
class DeployPySparkScriptOnAws(object):
    ...
    def __init__(self):
        ...
        self.s3_bucket_temp_files = "bucket-temp-files"     # S3 Bucket to store temporary files generated by this script
        self.s3_region = 's3-eu-west-1.amazonaws.com'       # S3 region to specifiy s3Endpoint in s3-dist-cp step
        self.user = 'thom'                                  #

    def run(self):
        session = boto3.Session(profile_name='ds')          # Select AWS IAM profile
        ...
        self.upload_temp_files(s3)                          # Move the Spark files we want to execute to a S3 bucket for temporary files
        c = session.client('emr')                           # Open EMR connection
        self.start_spark_cluster(c)                         # Start Spark EMR cluster
        self.step_spark_submit(c)                           # Add step
        self.describe_status_until_terminated(c)            # Describe cluster status until steps are finished (i.e. state=TERMINATED)
        self.remove_temp_files(s3)                          # Remove files from S3 bucket where we stored the temporary files in
    ...
    def temp_bucket_exists(self, s3):
        ...
        print("S3 bucket for temporary files exists")
    ...
    def upload_temp_files(self, s3):
        ...
        print("Uploaded files to key '{}' in bucket '{}'".format(self.job_name, self.s3_bucket_temp_files))
        return True
    ...
    def remove_temp_files(self, s3):
        ...
        for key in bucket.objects.all():
            if key.key.startswith(self.job_name) is True:
                key.delete()
                print("Removed '{}' from bucket for temporary files".format(key.key))
    ...
    def start_spark_cluster(self, c):
        ...
        print("Created Spark EMR-4.4.0 cluster with JobFlowId {}".format(self.job_flow_id))
    ...
    def step_spark_submit(self, c, arguments):
        ...
        print("Added step 'spark-submit' with argument '{}'".format(arg))
        time.sleep(1)  # Prevent ThrottlingException

    def step_copy_data_between_s3_and_hdfs(self, c, src, dest):
        """
        Copy data between S3 and HDFS
        :param c:
        :return:
        """
        response = c.add_job_flow_steps(
        ...
        print("Added step 'Copy data from {} to {}'".format(src, dest))
